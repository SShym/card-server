...
const endGameIfOver = (players, table) => {
  const activePlayers = Object.values(players).filter((player) => player.inGame === true);
  
  if (activePlayers.length === 1) {
    Object.values(players).forEach((player) => {
      player.role = null;
    });
    table.gameOver = [true, activePlayers[0].nickname];
  }
};
...
const gameOver = (players, table, loserName) => {
  Object.values(players).forEach((player) => {
    player.inGame = false;
    player.role = null;
    player.hand = [];
